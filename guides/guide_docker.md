...
3. Соберите оставшиеся образы ([Dockerfile_Payments](..%2FDockerfile_Payments))
...
> Для дальнейшей разработки нашего проекта поднимать приложение через докер не потребуется. Я поднимаю контейнер с БД, а само приложение запускаю через Идею.