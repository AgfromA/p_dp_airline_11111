...
class SearchControllerIT extends IntegrationTestBase {
    ...
    //    Поиск: рейс туда (2023-03-01) и рейс обратно (2023-04-06) для 2-х пассажиров
    ...
    void shouldReturnOneDirectReturnFlightWithFreeSeats() throws Exception {
        ...
        assertEquals(expDirDepartFlights + expDirReturnFlights, dataToCount+dataBackCount);
        assertNotNull(searchResult.getFlights().get(0).getDataBack());
        assertNotNull(searchResult.getFlights().get(0).getDataTo());
    ...
    //4. В базе: два прямых и два непрямых рейсов туда и два прямых и два непрямых рейсов
    ...
    void shouldReturnTwoDirectAndTwoNonDirectDepartAndTwoDirectAndTwoNonDirectReturnFlightsWithThreeSeatsOnEach()
        ...
        long dataToCount = searchResult.getFlights().stream().filter(flight -> flight.getDataTo() != null).count();
        long dataBackCount = searchResult.getFlights().stream().filter(flight -> flight.getDataBack() != null).count();
        // деление на два, так как на две карточки туда один и тот же полет обратно
    ...
    private void assertNumberOfReturnDirectFlights(int expectDirect, Search search,
                                                   String json) throws JsonProcessingException {
        var searchResult = getSearchResult(json);
// Посчитать количество прямых флайтов обратно
        int numberOfDirectReturnFlights = 0;
        Set<String> uniqueDataBackFlights = new HashSet<>();
        for (int i = 0; i < searchResult.getFlights().size(); i++) {
            String flightKey = searchResult.getFlights().get(i).getDataBack().getAirportFrom() + "-" + searchResult.getFlights().get(i).getDataBack().getAirportTo();
            if (!uniqueDataBackFlights.contains(flightKey)) {
                uniqueDataBackFlights.add(flightKey);
                if (searchResult.getFlights().get(i).getDataBack().getAirportFrom().equals(search.getTo()) &&
                        searchResult.getFlights().get(i).getDataBack().getAirportTo().equals(search.getFrom())) {
