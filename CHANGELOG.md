...
## [Unreleased]
- #282 Добавлен View для Aircraft(в т.ч. клиент feign). Entity, DTO и API перенесено в Common (Для Aircraft и Seat). DTO использует Mapper. Метод createAircraft() в API Теперь возвращает DTO вместо Entity
